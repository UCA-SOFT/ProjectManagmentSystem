    ...
    <div class="modal-content">
                ...
                <div class="row">
                    ...
                    <div class="col-6 start-div text-right">
                        @if($task->time_tracking == 0)
                            <a href="#" class="start-task timer form-control-label" data-type="start" data-id="{{$task->id}}"><i class="far fa-clock"></i> {{__('Start Tracking')}}</a>
                        @else
                            <a href="#" class="stop-task timer form-control-label" data-type="stop" data-id="{{$task->id}}"><i class="far fa-clock"></i> {{__("Stop Tracking")}}</a>
                        @endif
                    </div>
                </div>
                <hr class="my-3"/>
                                ...
                                <div class="col-auto">
                                    <a href="#" class="avatar avatar-sm">
                                        @if($activity->user->avatar)
                                            <img  src="{{$logo_path.$activity->user->avatar}}" class="avatar avatar-sm rounded-circle" >
                                               @else
                ...
                <div class="list-group list-group-flush mb-0">
                    @if($task->trackingRec->count())
                        @foreach($task->trackingRec as $track)
                            <div class="list-group-item px-0">
                                <span class="text-dark text-sm">{{ __($track->taskTime($track->start_time,$track->end_time)) }}</span>
                                <span class="text-sm">({{ \App\Models\Utility::getDateFormated($track->start_time,true).' '.__('to').' '.\App\Models\Utility::getDateFormated($track->end_time,true) }})</span>
                            </div>
                        @endforeach
                    @else
                                ...
                                <div class="col-auto">
                                    <a href="#" class="avatar avatar-sm rounded-circle">
                                        @if($comment->user->avatar)
                                            <img  src="{{$logo_path.$comment->user->avatar}}" class="avatar avatar-sm rounded-circle" >
                                        @else
                                            <img {{ $comment->user->img_avatar }} title="{{ $comment->user->name }}" class="avatar avatar-sm rounded-circle">
                                        @endif
                                    </a>
                                </div>
                                <div class="col ml-n2">
        ...
        <div class="modal-footer">
            <div class="col-12 d-flex">
                <div class="pr-3">
                    @if( Auth::user()->avatar)
                        <img  src="{{$logo_path.Auth::user()->avatar}}" class="avatar rounded-circle avatar-sm" >
                    @else
            ...
            currentRequest = $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data.status == 'success') {
                        if (type == 'start') {
                            main_div.find('.start-div').html('<a href="#" class="stop-task timer form-control-label" data-type="stop" data-id="' + id + '"><i class="far fa-clock"></i> {{__("Stop Tracking")}}</a>');
                            TrackerTimer(data.start_time);
                        } else {
                            main_div.find('.start-div').html(' <a href="#" class="start-task timer form-control-label" data-type="start" data-id="' + id + '"><i class="far fa-clock"></i> {{__('Start Tracking')}} </a>');
