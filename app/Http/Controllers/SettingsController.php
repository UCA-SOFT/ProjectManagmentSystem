...
class SettingsController extends Controller
{
    public function index()
    {
        if(Auth::user()->type == 'admin')
        {
            ...
            return view('users.setting', compact('timezones','settings'));
        }
        else
        {
    ...
    public function store(Request $request)
    {
        ...
        if($usr->type == 'admin')
        {
            ...
            if($request->favicon)
            {
                ...
                $path = Utility::upload_file($request,'favicon',$faviconName,$dir,$validation);
            ...
            if($request->full_logo)
            {
               // $request->validate(['full_logo' => 'required|image|mimes:png|max:2048']);
                $logoName = 'logo.png';
                $validator = \Validator::make($request->all(),['full_logo' => 'required|image|mimes:png',]);
                if($validator->fails())
                {
                    $messages = $validator->getMessageBag();
                    return redirect()->back()->with('error', "logo image must have  png file.");
                ...
                $path = Utility::upload_file($request,'full_logo',$logoName,$dir,$validation);
        ...
        else
        {
            ...
            if($request->from == 'invoice_setting')
            {
                ...
                if($request->light_logo)
                {
                    ...
                    //$request->validate(['light_logo' => 'required|image|mimes:png']);

                    $validator = \Validator::make($request->all(), [ 'light_logo' => 'required|image', ]);
                    if($validator->fails())
                    {
                        $messages = $validator->getMessageBag();
                        return redirect()->back()->with('error', "logo image must have  png file.");
                    ...
                    $path = Utility::upload_file($request,'light_logo',$light_logo,$dir,$validation);
                    if($path['flag'] == 1)
                    {
                        $light_logo = $path['url'];
                    }
                ...
                if($request->dark_logo)
                {
                    ...
                    $path = Utility::upload_file($request,'dark_logo',$dark_logo,$dir,$validation);
                    if($path['flag'] == 1)
                    {
                        $dark_logo = $path['url'];
                    }
