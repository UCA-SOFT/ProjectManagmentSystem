...
class TimesheetController extends Controller
{
    ...
    public function timesheetStore(Request $request)
    {
        ...
        if($project)
        {
            ...
            $project_task = ProjectTask::find($request->task_id);

            if($project_task){
                $assign_user = explode(',', $project_task->assign_to);
        
                $userdatas = User::whereIn('id', $assign_user)->get();
    
                foreach($userdatas as  $userdata){
        
                    $resp           = Utility::sendEmailTemplate('Create Timesheet', [$userdata->id => $userdata->email], $tArr, $request->project_id);
                }
